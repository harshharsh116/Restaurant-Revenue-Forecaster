...
with st.sidebar:
    with st.container(border=True):
        ...
        st.markdown("Supported: **CSV**, **Excel** (.xlsx, .xls), max 200 MB")

        uploaded_file = st.file_uploader(
            label="Drag & drop or click to browse",
            type=["csv", "xlsx", "xls"],
            accept_multiple_files=False,  # ← change to False if you want single file only
            key="sales_uploader"
        )
    ...
    if uploaded_file is not None:
        ...
        if c2:
            ...
            st.write("Total size", xscale.shape)

        model_name = st.sidebar.selectbox("Select model:--",
                                          ['Linear', 'Ridge', 'Lasso', 'Polynomial', 'Xgboost', 'SVR', 'Elastic net',
                                           'GridSearch'])
