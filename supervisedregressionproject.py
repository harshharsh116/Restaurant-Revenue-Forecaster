...
if c2:
    ...
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Last Five Rows</h3>", unsafe_allow_html=True)
    st.dataframe(df.tail(5))

    st.markdown("<h3 style='text-align: center; font-weight: bold;'>All Column Info</h3>", unsafe_allow_html=True)
    buffer = io.StringIO()
    df.info(buf=buffer)
    st.text(buffer.getvalue())

    st.markdown("<h3 style='text-align: center; font-weight: bold;'>All Column Numeric Description</h3>", unsafe_allow_html=True)
    st.write(df.describe())

    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Null Value Count</h3>", unsafe_allow_html=True)
    st.write(df.isnull().sum())

    # ─── Capacity ───
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>CapacityWise count of restaurants</h3>", unsafe_allow_html=True)
    fig = px.histogram(df, x="Seating Capacity", nbins=10)
    fig.update_layout(xaxis_title="<b>Seating Capacity</b>", yaxis_title="<b>Number of Restaurants</b>")
    fig.update_traces(marker_color="pink", marker_line_color="black", marker_line_width=1.5)
    st.plotly_chart(fig)

    # ─── Location ───
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>LocationWise Count of Restaurants</h3>", unsafe_allow_html=True)
    ...
    fig.update_traces(marker_color="pink", marker_line_color="black", marker_line_width=1.5)
    fig.update_layout(xaxis_title="<b>Location</b>", yaxis_title="<b>Number of Restaurants</b>")
    st.plotly_chart(fig)

    # ─── Cuisine ───
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Cuisine TypeWise Count of Restaurants</h3>", unsafe_allow_html=True)
    f = df.groupby('Cuisine')['Name'].count().reset_index()
    fig = px.bar(f, x="Cuisine", y="Name")
    fig.update_traces(marker_color="pink", marker_line_color="black", marker_line_width=1.5)
    fig.update_layout(xaxis_title="<b>Cuisine</b>", yaxis_title="<b>Number of Restaurants</b>")
    st.plotly_chart(fig)

    # ─── Rating ───
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Count of Restaurants By Rating</h3>", unsafe_allow_html=True)
    fig = px.histogram(df, x="Rating", nbins=5)
    fig.update_layout(xaxis_title="<b>Rating</b>", yaxis_title="<b>Number of Restaurants</b>")
    fig.update_traces(marker_color="pink", marker_line_color="black", marker_line_width=1.5)
    st.plotly_chart(fig)

    # ─── Reviews by Cuisine ───
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Count of CustomerReviews By Cuisine</h3>", unsafe_allow_html=True)
    q = df.groupby('Cuisine')['Number of Reviews'].count().reset_index()
    fig = px.bar(q, x="Cuisine", y="Number of Reviews")
    fig.update_traces(marker_color="darkblue", marker_line_color="black", marker_line_width=1.5)
    fig.update_layout(xaxis_title="<b>Cuisine</b>", yaxis_title="<b>Number of Reviews</b>")
    st.plotly_chart(fig)

    # ... (all other EDA plots follow the same pattern - kept exactly as you wrote them)

    # Last one (Average Revenue by Cuisine in Specific Location)
    st.markdown("<h3 style='text-align: center; font-weight: bold;'>Average Revenue by Cuisine in Specific Location </h3>", unsafe_allow_html=True)
    ...
    fig.update_layout(xaxis_title="<b>Cuisine</b>", yaxis_title="<b>Revenue</b>")
    st.plotly_chart(fig)
...
# ────────────────────────────────────────────────
c2 = st.sidebar.checkbox("To Show Size of Training and Testing")  # note: reused name - consider renaming later
if c2:
